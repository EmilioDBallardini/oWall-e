...
namespace UnityStandardAssets.Cameras
{
    ...
    public class AutoCam : PivotBasedCameraRig
    {
        ...
        [SerializeField] private float myTargetVelocityLowerLimit = 4f;// the minimum velocity above which the camera turns towards the object's velocity. Below this we use the object's forward direction.
        ...
        new public Transform myTarget;
        protected override void FollowTarget(float deltaTime)
        {
            
            // if no target, or no time passed then we quit early, as there is nothing to do
            if (!(deltaTime > 0) || myTarget == null)
            {
                return;
            }

            // initialise some vars, we'll be modifying these in a moment
            var targetForward = myTarget.forward;
            var targetUp = myTarget.up;
            ...
            if (m_FollowVelocity && Application.isPlaying)
            {
                ...
                if (targetRigidbody.velocity.magnitude > myTargetVelocityLowerLimit)
                {
            ...
            transform.position = Vector3.Lerp(transform.position, myTarget.position, deltaTime*m_MoveSpeed);
